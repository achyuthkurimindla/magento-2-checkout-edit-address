...
namespace Checkout\EditForm\Controller\Address;
...
class FormPost extends \Magento\Customer\Controller\Address implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        // $new_url = $_GET['checkoutform'];
        $formId = $this->getRequest()->getPost('checkoutform');
        ...
        try {
            ...
            //  }
            if ($formId == 'form1') {
                $url=$this->_buildUrl('checkout/index', ['_secure' => true]);
            } else{
                $url = $this->_buildUrl('*/*/index', ['_secure' => true]);
            }
            ...
            if ($formId == 'form1') {
                $redirectUrl = $this->_buildUrl('checkout/index');
            } else{
        ...
        if (!$redirectUrl) {
            $this->_getSession()->setAddressFormData($this->getRequest()->getPostValue());
            if ($formId == 'form1') {
                $url = $this->_buildUrl('checkout/index', ['id' => $this->getRequest()->getParam('id')]);
            } else{
